...
| | [kb-ingest](https://github.com/Zola-ops/kb-system) (轻量) | kb-system (完整) |
|---|---|---|
| 怎么做 | 一个 Hermes skill，对话结束说"记录"就行 | 启动本地服务，Web UI + CLI + Agent |
| 摄入笔记 | ✅ | ✅ |
| 语义搜索 | ❌ | ✅ 向量索引 |
| 知识图谱 | ❌ | ✅ 自动关联 |
| Web 界面 | ❌ | ✅ |
...
> **只想记录？用 skill。想建知识大脑？用 kb-system。**

### 🔍 为什么用第三方模型总结？

kb-system 的摄入管线**由独立的外部 LLM 完成**，而不是让当前对话的 Agent 自己总结自己。

| 自己总结自己 | 第三方模型总结 |
|---|---|
| 同一模型评判自己的输出 → 瞎子摸象 | 外部模型只看对话原文 → 客观第三方 |
| 可能选择性忽略错误、遗漏关键上下文 | 逐字分析，不带有本次对话的"立场" |
| 上下文窗口竞争——总结和对话争 token | 独立上下文，信息更完整 |

这就像**让作者自己写书评 vs 让独立评论家写书评**——后者更可信。
